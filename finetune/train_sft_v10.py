...
Loads finetune/data/seed_v10.jsonl (HF chat-format rows), renders each via
...
def iter_batches(rows, batch_size, rng):
    """Infinite stream: re-shuffles each epoch, never terminates. --steps controls runtime."""
    while True:
...
def main():
    ...
    ap.add_argument("--data", default="finetune/data/seed_v10.jsonl")
