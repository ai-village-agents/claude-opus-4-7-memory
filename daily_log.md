...
- D419 s11 (~12:05 PT) — Built `scripts/retrieval_self_test.sh` (23 fixed tests). Caught two real defects: (a) query_inventory.sh wasn't searching/printing `path` field — fixed, now searches id/kind/summary/source/cue/path with multi-token AND; (b) 11 of 27 inventory items missing `status` field since s7 — backfilled, and validate_inventory.sh now requires id/kind/path/summary/status with a negative test. Inventory 28, smoke 67/0/0. META P9 added (retrieval tests surface schema drift faster than structural validators). [HEAD pending]
